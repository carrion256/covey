...
use crate::{
    ...
    model::{
        Claim, ClaimState, ClaimableSubtaskAvailability, CompletionPolicy, ExpiringClaim,
        RoutingKey, Session, SessionRole, SessionState, StuckSubtask, SubtaskCandidate,
        SubtaskKind, SubtaskRow, SubtaskState, SubtaskStatus, SubtaskView, claim_state_name,
        meta_task_state_name, subtask_kind_name, subtask_state_name,
    },
    queries::{
        load_artifact_tx, load_attempt_outcomes_for_subtask_tx, load_claim_tx,
        load_queue_items_for_subtask_tx, load_reviews_for_subtask_tx, load_subtask_tx,
    },
};
...
impl Covey {
    ...
    pub fn subtask_status(&self, subtask_id: &str) -> Result<SubtaskStatus> {
        ...
        let result = self.with_read_tx(|tx| {
            ...
            let artifact = subtask
                ...
                .transpose()?;
            let attempt_outcomes = load_attempt_outcomes_for_subtask_tx(tx, subtask_id)?;
            ...
            SubtaskStatus::new_with_landing_receipt_and_attempt_outcomes(
    ...
    /// Returns claimable counts within one exact routing lane.
    pub fn claimable_subtask_availability_routed(
        &self,
        ...
        let result = self.with_read_tx(|tx| {
            let executor_claimable_count =
                claimable_subtask_count_tx(tx, SubtaskKind::Work, routing_key, meta_task_id)?;
            let reviewer_claimable_count =
                claimable_subtask_count_tx(tx, SubtaskKind::Review, routing_key, meta_task_id)?;
    ...
    /// Returns ordered candidates within one exact routing lane.
    pub fn subtask_candidates_routed(
        &self,
        role: SessionRole,
        routing_key: &RoutingKey,
        limit: usize,
        ...
        let result = if limit == 0 {
            ...
            self.with_read_tx(|tx| match role {
                SessionRole::Executor => subtask_candidates_tx(
                    tx,
                    SubtaskKind::Work,
                    routing_key,
                    meta_task_id,
                ...
                SessionRole::Reviewer => subtask_candidates_tx(
                    tx,
                    SubtaskKind::Review,
                    routing_key,
                    meta_task_id,
        ...
        let result = self.with_read_conn(|conn| {
            let mut stmt = conn.prepare(
                ...
                SELECT st.subtask_id, st.meta_task_id, st.title, st.kind,
                       ...
                       st.completion_policy, st.routing_key, st.created_at, st.updated_at,
                ...
                WHERE st.state NOT IN (
                    'available', 'applied', 'completed', 'failed', 'abandoned', 'decided'
                )
                  AND st.updated_at <= ?1
                  AND NOT (
...
fn subtask_candidates_tx(
    ...
    match kind {
        SubtaskKind::Work => subtask_work_candidates_tx(tx, routing_key, meta_task_id, limit, now),
        SubtaskKind::Review => subtask_review_candidates_tx(tx, routing_key, meta_task_id, limit),
...
fn subtask_work_candidates_tx(
    ...
    let dependency_clause = r#"
        ...
        LIMIT ?7
    "#;
    let scoped_clause = " AND s.meta_task_id = ?8\n";
...
fn subtask_review_candidates_tx(
    ...
    let base_select = r#"
        ...
        WHERE s.kind = ?1
          ...
          AND s.routing_key = ?5
    "#;
    let scoped_clause = " AND s.meta_task_id = ?7\n";
    let order_clause = " ORDER BY s.priority ASC, s.created_at ASC LIMIT ?6";
    let sql = if meta_task_id.is_some() {
...
fn candidate_from_row(row: &Row<'_>) -> Result<SubtaskCandidate> {
    let blocked_dependents_count = row.get::<_, i64>(13)?;
    SubtaskCandidate::new_with_execution_contract(
        row.get(0)?,
        row.get(1)?,
        row.get(2)?,
        ...
        row.get(7)?,
        row.get(8)?,
        parse_enum::<CompletionPolicy>(row.get::<_, String>(9)?)?,
        row.get(10)?,
        row.get(11)?,
        row.get(12)?,
...
fn stuck_subtask_from_joined_row(row: &Row<'_>, now: i64) -> Result<StuckSubtask> {
    let subtask = SubtaskRow::try_from_parts(
        ...
        row.get(8)?,
        row.get(9)?,
        parse_enum::<CompletionPolicy>(row.get::<_, String>(10)?)?,
        row.get(11)?,
        row.get(12)?,
        row.get(13)?,
    )?;
    ...
    let claim_id = row.get::<_, Option<crate::model::ClaimId>>(14)?;
    let claim = claim_id
        .map(|claim_id| {
            Claim::try_from_parts(
                ...
                row.get(17)?,
                row.get(18)?,
                parse_enum(row.get::<_, String>(19)?)?,
                row.get(20)?,
                row.get(21)?,
            )
            .map_err(to_sql_err)
    ...
    let session_token = row.get::<_, Option<crate::model::SessionToken>>(22)?;
    let session = session_token
        .map(|session_token| {
            Session::try_from_parts(
                ...
                row.get::<_, String>(23)?,
                row.get::<_, String>(24)?,
                parse_enum::<SessionRole>(row.get::<_, String>(25)?)?,
                parse_enum::<SessionState>(row.get::<_, String>(26)?)?,
                row.get(27)?,
                row.get(28)?,
                row.get(29)?,
                row.get(30)?,
...
fn expiring_claim_from_joined_row(row: &Row<'_>, lease_now: i64) -> Result<ExpiringClaim> {
    ...
    let subtask = SubtaskRow::try_from_parts(
        ...
        row.get(16)?,
        row.get(17)?,
        parse_enum::<CompletionPolicy>(row.get::<_, String>(18)?)?,
        row.get(19)?,
        row.get(20)?,
        row.get(21)?,
    )?;
