...
use better_droid::{
    BetterDroidError, CompileOptions, DoctorOptions, LintOptions, compile_change, doctor_change,
    lint_change,
};

#[cfg(test)]
use better_droid::{DoctorCheckStatus, DoctorStatus, ReportStatus, run_openspec_check};
...
enum Command {
    ...
    Compile {
        /// OpenSpec change id under openspec/changes/.
        change_id: String,
        /// Optional generated output directory. It must stay inside the project and outside openspec/changes/*/mission.
        #[arg(long)]
...
fn run(raw_args: Vec<OsString>) -> Result<ExitCode, CliError> {
    ...
    match cli.command {
        ...
        Command::Doctor {
            ...
            let report = doctor_change(DoctorOptions {
                project_root: cli.project_root,
                change_id,
                openspec_bin,
            })
...
mod tests {
    ...
    fn doctor_status_display_and_failed_openspec_execution_are_stable() {
        assert_eq!(
            DoctorStatus::CoveyImportReady.to_string(),
            "covey_import_ready"
        );
        assert_eq!(DoctorStatus::PlanningReady.to_string(), "planning_ready");
        assert_eq!(
            DoctorStatus::PlanningBlocked.to_string(),
            "planning_ready_blocked"
        );
    ...
    #[test]
    fn doctor_status_preserves_domain_when_openspec_checks_pass() {
        assert_eq!(
            DoctorStatus::from_report_status(ReportStatus::PlanningReady, true),
            DoctorStatus::PlanningReady
        );
        assert_eq!(
            DoctorStatus::from_report_status(ReportStatus::PlanningBlocked, true),
            DoctorStatus::PlanningBlocked
        );
        assert_eq!(
            DoctorStatus::from_report_status(ReportStatus::CoveyImportReady, true),
            DoctorStatus::CoveyImportReady
        );
        assert_eq!(
            DoctorStatus::from_report_status(ReportStatus::PlanningReady, false),
